...
(defcustom inkscape-thumbnail-width 300
  "Width of thumbnails in pts."
  :group 'inkscape)

(defcustom inkscape-template-svg
...
Make a new file if needed."
  ...
  (unless (file-exists-p path)
    (with-temp-file path
      (insert inkscape-template-svg)))
  (shell-command (format "inkscape %s &" path)))

(defun inkscape-thumbnail (start end path bracketp)
...
(org-link-set-parameters
 "inkscape"
 :follow 'inkscape-open
 :help-echo "Click to open in inkscape."
 :activate-func 'inkscape-thumbnail
...
(defun inkscape-image-path (name)
  "Convenience function to insert a drawing with filename PATH."
  ;; basedir + prefix + path
  (let ((basedir (funcall #'projectile-project-root))
        (fname (funcall #'file-name-base))
        (prefix  inkscape-image-prefix)
        savedir)
    (setq savedir (concat basedir prefix fname))
    (make-directory savedir :parents)
    (file-relative-name (expand-file-name name savedir))))
